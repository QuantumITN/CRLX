...
function runSync(array $buildings, array &$syncMeta, ?DateTimeImmutable $anchorMonth = null): void
{
    ...
    if (!empty($plCfg['enabled']) && $plKey !== '') {
        $baseMonth = $anchorMonth ?: new DateTimeImmutable('first day of this month', new DateTimeZone('UTC'));
        $fromDate = $baseMonth->modify('-1 month')->format('Y-m-01');
        $toDate = $baseMonth->modify('+2 month')->modify('-1 day')->format('Y-m-d');
