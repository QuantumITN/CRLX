...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (verifyAdminCredentials($user, $pass)) {
        ...
        $jsCookie = buildAuthCookieJavascript(getAdminUsername());
        $safeRedirect = htmlspecialchars($redirect, ENT_QUOTES, 'UTF-8');
        echo '<script>document.cookie=' . json_encode($jsCookie) . ';window.location.replace(' . json_encode($redirect) . ');</script>';
        echo '<noscript><meta http-equiv="refresh" content="0;url=' . $safeRedirect . '"></noscript>';
        exit;
    }
