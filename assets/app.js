(() => {
  ...
  function buildBoard() {
    ...
    const frame = scheduler.closest('.scheduler-frame');
    labelWidth = window.innerWidth <= 900 ? 160 : 220;
    if (frame) {
      const frameWidth = frame.clientWidth || 0;
      const available = Math.max(320, frameWidth - labelWidth);
      const fitted = Math.floor(available / Math.max(1, monthDays));
      dayWidth = Math.max(32, fitted);
    } else {
      dayWidth = 40;
    }
    ...
    scroller.style.setProperty('--dayw', `${dayWidth}px`);
    scroller.style.setProperty('--labelw', `${labelWidth}px`);
  ...
  function placeReservations() {
    ...
    allReservations.forEach((r) => {
      ...
      bar.dataset.id = r.id;
      bar.style.left = `${startOffset * dayWidth + 2}px`;
  ...
  function enableDrag(el, reservation) {
    ...
    let previewEl = null;
    let proxyEl = null;
    let pendingStartOffset = null;

    const clearPreview = () => {
      if (hoverRow) {
        hoverRow.classList.remove('drop-target-row');
      }
      hoverRow = null;
    ...
    const clearProxy = () => {
      if (proxyEl && proxyEl.parentElement) {
        proxyEl.parentElement.removeChild(proxyEl);
      }
      proxyEl = null;
    };
    ...
    el.addEventListener('pointerdown', (ev) => {
      ...
      startX = ev.clientX;
      moved = false;
      pendingStartOffset = Math.max(0, Math.round((parseFloat(el.style.left || '0') - 2) / dayWidth));
    ...
    el.addEventListener('pointermove', (ev) => {
      ...
      if (hoverRow !== rowEl) {
        clearPreview();
        hoverRow = rowEl;
        hoverRow.classList.add('drop-target-row');
      }
      ...
      const width = Math.max(8, oldLen * dayWidth - 4);

      if (!previewEl) {
        previewEl = document.createElement('div');
        previewEl.className = 'bar bar-drop-preview';
        previewEl.textContent = 'Drop here';
        track.appendChild(previewEl);
      }

      previewEl.style.left = `${startOffset * dayWidth + 2}px`;
    ...
    const finish = async (ev) => {
      ...
      el.classList.remove('bar-origin-shadow');
      clearProxy();
